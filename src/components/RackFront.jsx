...
export default function RackFront({ config }) {
  ...
  return (
    <div className="flex justify-center">
      <svg width={width} height={height} style={{ border: '2px solid #5A4A3A', background: '#C8B8A8' }}>
        <defs>
          <linearGradient id="woodGradFront" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" style={{ stopColor: '#9A8A7A', stopOpacity: 1 }} />
            <stop offset="100%" style={{ stopColor: '#6A5A4A', stopOpacity: 1 }} />
          </linearGradient>
        </defs>

        <rect width={width} height={height} fill="url(#woodGradFront)" />
        ...
        {config.components.map((compKey) => {
          ...
          return (
            ...
            >
              ...
              <circle cx={pos.x + 8} cy={pos.y + comp.height + 6} r="2" fill="#DAA" />
              <circle cx={pos.x + comp.width - 8} cy={pos.y + comp.height + 6} r="2" fill="#DAA" />
